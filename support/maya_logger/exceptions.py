...
class MayaException(Exception):
  """
  Excepción lanzada para errores de Maya
  Permite la grabación de la excepción en el log y en la base de datos de Maya
  ...
  def __init__(self, logger, msg = '', level = 40, level_name = '', comments = '', stack = False, toFile = False):
    ...
    if len(self.level_name) == 0:
      self.level_name = MayaException.LOG_LEVELS[level]
